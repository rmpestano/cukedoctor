...
public class CukedoctorConverterTest {
  ...
  public void initProperties() {
    System.setProperty(
        "HIDE_STEP_TIME", GlobalConfig.getInstance().getLayoutConfig().isHideStepTime() + "");
    System.setProperty(
        "HIDE_FEATURES_SECTION",
        GlobalConfig.getInstance().getLayoutConfig().isHideFeaturesSection() + "");
    System.setProperty(
        "HIDE_SUMMARY_SECTION",
        GlobalConfig.getInstance().getLayoutConfig().isHideSummarySection() + "");
    System.setProperty(
        "HIDE_SCENARIO_KEYWORD",
        GlobalConfig.getInstance().getLayoutConfig().isHideScenarioKeyword() + "");
  ...
  public void shouldGeneratePdfTheme() {
    ...
    CukedoctorConverter converter =
        Cukedoctor.instance(features, new DocumentAttributes().backend("pdf"));
    converter.setFilename("target/pdf/living documentation.adoc");
    String pdfStylePath =
        Paths.get("").toAbsolutePath().toString().replace("\\", "/") /* Windows path hack */
            + "/target/pdf/cukedoctor-pdf.yml";
  ...
  public void shouldNotGeneratePdfThemeForHtmlBackend() {
    ...
    features.add(feature);
    CukedoctorConverter converter =
        Cukedoctor.instance(features, new DocumentAttributes().backend("html"));
    converter.setFilename("/target/pdf//living documentation.adoc");
    converter.addCustomPdfTheme();
    assertThat(FileUtil.loadFile("target/pdf/cukedoctor-pdf.yml")).doesNotExist();
  }
  ...
  public void shouldNotSetInvalidFilename() {
    ...
    features.add(feature);
    CukedoctorConverter converter =
        Cukedoctor.instance(features, new DocumentAttributes().docTitle("Doc Title"));
  ...
  public void shouldRenderFeatureDescriptionWithNewLines() {
    List<Feature> features =
        FeatureParser.parse(FileUtil.findJsonFile("target/test-classes/json-output/sample.json"));
    ...
    assertThat(trimAllLines(resultDoc))
        .contains(
            "****"
                + newLine()
                + "As a user"
  ...
  public void shouldSaveDocumentationIntoDisk() {
    ...
    assertThat(FileUtil.loadFile("target/generated-test-sources/living_documentation.adoc"))
        .exists();
  }
  ...
  public void shouldNotWriteMinmaxMacroWhenExtensionIsDisabled() {
    ...
    List<Feature> features = FeatureParser.parse(onePassingOneFailing);
    String resultDoc =
        Cukedoctor.instance(features, new DocumentAttributes().docTitle("Title"))
            .renderDocumentation();
    assertThat(resultDoc).isNotNull().doesNotContain("minmax");
  ...
  public void shouldWriteMinmaxMacroWhenExtensionIsEnabled() {
    ...
    String resultDoc =
        Cukedoctor.instance(features, new DocumentAttributes().docTitle("Title"))
            .renderDocumentation();
    assertThat(resultDoc).isNotNull().contains("minmax");
  }
  ...
  public void shouldRenderIntroChapterUsingAbsolutePath() {
    List<Feature> features = FeatureParser.parse(onePassingOneFailing);
    String resultDoc =
        Cukedoctor.instance(features, new DocumentAttributes()).renderDocumentation();
    assertThat(resultDoc)
        .containsOnlyOnce(
            "/cukedoctor-converter/target/test-classes/cukedoctor-intro.adoc[leveloffset=+1]");
  }
    ...
    try {
      ...
      assertThat(resultDoc)
          .doesNotContain(
              "/cukedoctor-converter/target/test-classes/cukedoctor-intro.adoc[leveloffset=+1]");
      assertThat(resultDoc)
          .containsOnlyOnce("target/test-classes/cukedoctor-intro.adoc[leveloffset=+1]");
