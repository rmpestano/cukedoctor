...
public class CukedoctorFeatureRendererTest {
  ...
  public void shouldNotRenderFeatureWithSkipDocsTag() {
    ...
    String resultDoc =
        new CukedoctorFeatureRenderer(new DocumentAttributes())
            .renderFeatures(
                features, CukedoctorDocumentBuilder.Factory.newInstance().createNestedBuilder());
  ...
  public void shouldEnrichFeature() {
    List<Feature> features =
        FeatureParser.parse(getClass().getResource("/json-output/enrichment/calc.json").getPath());
    assertThat(features).isNotNull().hasSize(1);
    String output =
        new CukedoctorFeatureRenderer((DocumentAttributes) null)
            .renderFeatures(
                features, CukedoctorDocumentBuilder.Factory.newInstance().createNestedBuilder());
  ...
  public void shouldEnrichFeatureWithListing() {
    List<Feature> features =
        FeatureParser.parse(
            getClass()
                .getResource("/com/github/cukedoctor/json-output/comment-with-listing.json")
                .getPath());
    assertThat(features).isNotNull().hasSize(1);
    String output =
        new CukedoctorFeatureRenderer((DocumentAttributes) null)
            .renderFeatures(
                features, CukedoctorDocumentBuilder.Factory.newInstance().createNestedBuilder());
    assertThat(output.replaceAll("\r", ""))
        .contains(
  ...
  public void shouldEnrichFeatureWithListingWithinAdmonitionBlock() {
    List<Feature> features =
        FeatureParser.parse(
            getClass()
                .getResource(
                    "/com/github/cukedoctor/json-output/comment-with-admonition-and-listing.json")
                .getPath());
    assertThat(features).isNotNull().hasSize(1);
    String output =
        new CukedoctorFeatureRenderer((DocumentAttributes) null)
  ...
  public void shouldEnrichFeatureWithCommentAndDocstring() {
    List<Feature> features =
        FeatureParser.parse(
            getClass()
                .getResource("/com/github/cukedoctor/json-output/calc-enriched.json")
                .getPath());
    assertThat(features).isNotNull().hasSize(1);
    String output =
        new CukedoctorFeatureRenderer((DocumentAttributes) null)
  ...
  public void shouldRenderFeatureScenarios() {
    ...
    String resultDoc =
        featureRenderer.renderFeatureScenarios(
            feature,
            new CukedoctorDocumentBuilderImpl().createNestedBuilder().createNestedBuilder());
    assertThat(resultDoc)
        .isEqualTo(
            "==== Scenario: scenario 1"
                + newLine()
  ...
  public void shouldRenderFeatureScenariosWithTagsInFeaturesAndScenarios() {
    ...
    assertThat(resultDoc)
        .isEqualTo(
            "==== Scenario: scenario 1"
                + newLine()
                + "[small]#tags: @tag2,@FeatureTag,@Tag1#"
                + newLine()
                + ""
                + newLine()
                + "description"
  ...
  public void shouldRenderFeatureScenariosWithMultipleSteps() {
    ...
    assertThat(resultDoc.replaceAll("\r", ""))
        .isEqualTo(
            ("==== Scenario: scenario icon:thumbs-down[role=\"red\",title=\"Failed\"]"
                    + newLine()
                    + "description"
                    + newLine()
                    + ""
  ...
  public void shouldRenderSourceDocStringInStep() {
    ...
    CukedoctorFeatureRenderer featureRenderer =
        new CukedoctorFeatureRenderer(new DocumentAttributes());
    String resultDoc =
  ...
  public void shouldRenderFeatureDescription() {
    final Feature feature =
        FeatureBuilder.instance().description("Feature description").name("Feature name").build();

    CukedoctorFeatureRenderer featureRenderer =
        new CukedoctorFeatureRenderer(new DocumentAttributes());
    featureRenderer = spy(featureRenderer);
    doReturn("")
        .when(featureRenderer)
        .renderFeatureScenario(
            any(Scenario.class), eq(feature), any(CukedoctorDocumentBuilder.class));
    String resultDoc =
        featureRenderer.renderFeature(
            feature,
            new CukedoctorDocumentBuilderImpl().createNestedBuilder().createNestedBuilder());
  ...
  public void shouldNotTrimFeatureWithCodeListingInDescription() {
    final String descriptionWithListing =
        "[source, xml]"
            + newLine()
            + "----"
            + newLine()
            ...
            + newLine()
            + "----";
    final Feature featureWithCodeInDescription =
        FeatureBuilder.instance()
            .description(descriptionWithListing)
            .name("A feature with listing")
            .build();
    final String resultDoc =
        new CukedoctorFeatureRenderer(new DocumentAttributes())
            .renderFeature(
                featureWithCodeInDescription,
  ...
  public void shouldNotTrimScenarioWithCodeListingInDescription() {
    ...
    final Feature feature =
        FeatureBuilder.instance()
            ...
            .scenario(
                ScenarioBuilder.instance()
                    .name("Scenario with listing in description")
                    .description(descriptionWithListing)
                    .build())
            .build();
    final String resultDoc =
        new CukedoctorFeatureRenderer(new DocumentAttributes())
            .renderFeature(
                feature, CukedoctorDocumentBuilder.Factory.newInstance().createNestedBuilder());
    assertThat(resultDoc).contains(descriptionWithListing);
  }
  ...
  public void shouldRenderFeaturesIncludingSingleQuotes() {
    ...
    assertThat(resultDoc)
        .isEqualTo(
            "[[L-API-Open-API-est-disponible-sur-demande, "
                + "L'API Open API est disponible sur demande]]"
                + newLine()
                + "== *L'API Open API est disponible sur demande*"
                + newLine()
                ...
                + "ifndef::backend-pdf[]"
                + newLine()
                + "minmax::L-API-Open-API-est-disponible-sur-demande[]"
                + newLine()
                + "endif::[]"
                + newLine()
                + "****"
