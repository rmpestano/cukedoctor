...
public class StepDefs {
  ...
  public void renderFeatures(List<Feature> features) {
    renderedDocument =
        new SectionFeatureRenderer()
            .renderFeatures(features, CukedoctorDocumentBuilder.Factory.newInstance());
  }

  public void runCukedoctor(List<Feature> features) {
    CukedoctorConfig config = getConfig();
    renderedDocument =
        Cukedoctor.instance(features, new DocumentAttributes(), config).renderDocumentation();
  }
  ...
  private CukedoctorConfig getConfig() {
    return new CukedoctorConfig()
        ...
        .setIntroChapterDir(
            introDirectory.getAbsolutePath()) // Avoid picking up intro docs in source tree
